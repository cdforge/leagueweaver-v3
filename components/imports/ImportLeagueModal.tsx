...
import { AlertCircle, ArrowLeft, Check, FileSpreadsheet, LoaderCircle, ShieldCheck, Upload, X } from "lucide-react";
...
const SOURCE_META: Record<ImportSource, { title: string; description: string; icon: React.ReactNode }> = {
  ...
  espn: { title: "Import ESPN Public Beta", description: "Paste a public league URL or league ID. Private credentials are never accepted.", icon: <img src="/providers/espn.ico" alt="" /> },
...
function parsePastedRoster(value: string, provider: "csv" | "paste"): ImportPreview {
  ...
  const teams = dataLines.map((line, index) => {
    ...
    return {
      ...
      city: hasHeader ? at(["city", "location"], -1) : "",
      name: at(["team", "team name"], 0) || `Team ${index + 1}`,
      manager: at(["manager", "owner"], 1),
      division: at(["division"], 2),
      rank: Number(at(["rank", "overall rank"], 3)) || index + 1,
      stadium: at(["stadium", "venue"], 4),
  ...
  return {
    ...
    warnings: hasHeader ? [] : ["No headers were found, so columns were read as team, manager, division, rank, and venue."],
    requiresConfirmation: true,
  };
}

function TeamPreviewRow({ team, index, onChange }: { team: ImportTeam; index: number; onChange: (next: ImportTeam) => void }) {
  ...
  return (
    <div className="import-review-row">
      <span className="import-rank">{index + 1}</span>
      <IdentityColorPicker compact name={team.name} abbreviation={abbreviation} color={team.color ?? TEAM_COLORS[index % TEAM_COLORS.length]} logoUrl={team.logoUrl} onChange={(identity) => onChange({ ...team, ...identity })} />
      ...
      <input aria-label={`${team.name} division`} value={team.division ?? ""} placeholder="Division" onChange={(event) => onChange({ ...team, division: event.target.value })} />
      <input aria-label={`${team.name} rank`} type="number" min="1" value={team.rank ?? index + 1} onChange={(event) => onChange({ ...team, rank: Number(event.target.value) || index + 1 })} />
      <input aria-label={`${team.name} venue`} value={team.stadium ?? ""} placeholder="Home venue" onChange={(event) => onChange({ ...team, stadium: event.target.value })} />
    </div>
  );
}
      ...
      <section className="import-modal" role="dialog" aria-modal="true" aria-labelledby="import-modal-title">
        ...
        {!preview ? (
          <div className="import-modal-body">
            {(source === "sleeper" || source === "espn") && <div className="import-form-grid">
              <label><span>{source === "sleeper" ? "League ID or username" : "Public league URL or ID"}</span><input autoFocus value={identifier} onChange={(event) => setIdentifier(event.target.value)} placeholder={source === "sleeper" ? "Example: 123456789 or username" : "Paste ESPN league URL"} /></label>
          ...
          <div className="import-modal-body import-review">
            <div className="import-review-summary">
              <IdentityColorPicker compact name={preview.leagueName || setup.name} abbreviation={(preview.leagueName || setup.name).slice(0, 2).toUpperCase()} color={preview.leagueColor || setup.color} logoUrl={preview.leagueLogoUrl} onChange={(identity) => setPreview((current) => current ? ({ ...current, leagueColor: identity.color, leagueLogoUrl: identity.logoUrl }) : current)} />
              <div><span>{preview.provider.toUpperCase()} PREVIEW</span><h3>{preview.leagueName || "Imported league"}</h3><p>{preview.teams.length} teams found · Review names, logos, colors, and divisions.</p></div>
              <span className={`import-status ${supported ? "ready" : "blocked"}`}>{supported ? <><Check />Ready</> : <><AlertCircle />Needs edits</>}</span>
            </div>
            {(rosterMessage || preview.warnings.length > 0) && <div className="import-warning"><AlertCircle /><div>{rosterMessage && <strong>{rosterMessage}</strong>}{preview.warnings.map((warning) => <span key={warning}>{warning}</span>)}</div></div>}
            <div className="import-review-head"><span>#</span><span>Logo</span><span>City</span><span>Team name</span><span>Manager</span><span>Division</span><span>Rank</span><span>Venue</span></div>
            <div className="import-review-list">{preview.teams.map((team, index) => <TeamPreviewRow key={team.providerId ?? index} team={team} index={index} onChange={(next) => updateTeam(index, next)} />)}</div>
          </div>
        )}

        <footer className="import-modal-actions">
          <button type="button" className="button-secondary visible" onClick={preview ? () => setPreview(null) : onClose}>{preview ? <><ArrowLeft />Back</> : "Cancel"}</button>
          ...
          {preview && <button type="button" className="button-primary" disabled={!supported || preview.teams.some((team) => !team.name.trim())} onClick={() => onConfirm(preview)}><Check />Use this roster</button>}
        </footer>
      </section>
    </div>
  );
