...
import {
  WeekRecapWorkspace,
  getLatestFinalWeek,
} from "@/components/season/WeekRecapWorkspace";
...
function historySyncNotice(historySync?: HistorySyncResponse) {
  ...
  const yearText = years?.length
    ? ` Previous years found: ${years.join(", ")}.`
    : "";
  const warningText = historySync.warnings?.[0]
    ? ` ${historySync.warnings[0]}`
    : "";
  if ((historySync.rowsWritten ?? 0) > 0)
    return ` Historical data saved.${yearText}${warningText}`;
  return yearText || warningText
    ? ` Historical data checked.${yearText}${warningText}`
...
function buildHistoryTeamMap(
  ...
  const currentProviderTail = new Map<string, string>(
  ...
  const currentName = new Map<string, string>(
    schedule.setup.teams.map((team) => [
      normalizeHistoryName(`${team.city ?? ""} ${team.name}`),
      team.id,
    ]),
  );
...
function historySlot(
  provider: HistoryBrowserSeason["provider"],
  rawSlot: string,
) {
  const parsed = Number(rawSlot);
  if (provider === "espn" && Number.isInteger(parsed))
    return espnSlotKey(parsed);
...
function historicalFieldSize(
  season: HistoryBrowserSeason,
  fallback: PlayoffFieldSize,
) {
  const providerValue = numberFromHistorySetting(
    season.playoffSettings?.playoff_teams,
  );
...
function compareHistoryPlayerRows(
  provider: HistoryBrowserSeason["provider"],
  left: HistoryBrowserSeason["playerRows"][number],
  right: HistoryBrowserSeason["playerRows"][number],
) {
  ...
  return (
    ...
    left.lineupSlot.localeCompare(right.lineupSlot, undefined, {
      numeric: true,
    }) ||
    left.providerPlayerId.localeCompare(right.providerPlayerId, undefined, {
      numeric: true,
    })
  );
}
...
function buildHistoricalSchedule(
  ...
  for (const game of season.games) {
    ...
    const roundIndex =
      isPlayoff && regularWeeks ? game.week - regularWeeks - 1 : -1;
    const gameNumber = isPlayoff
      ? (playoffGamesByRound.get(roundIndex)?.length ?? 0) + 1
      : (gamesByWeek.get(game.week)?.length ?? 0) + 1;
    const baseGame = {
      id: isPlayoff
        ? `main-r${roundIndex + 1}-g${gameNumber}`
        : `history-${season.season}-${game.providerMatchupId}`,
      week: game.week,
      gameNumber,
      homeTeamId,
      awayTeamId,
      ...
      notes: [
        ...
        isPlayoff
          ? "Historical playoff game"
          : "Historical regular-season game",
      ],
  ...
  const weeks = [...gamesByWeek.entries()]
...
function buildHistoricalPlayerRows(
  schedule: GeneratedSchedule,
  season?: HistoryBrowserSeason,
  ...
  return sortedRows.flatMap((row) => {
    ...
    const starterIndex =
      lineupStatus === "starter"
        ? (starterCounts.set(
            starterKey,
            (starterCounts.get(starterKey) ?? 0) + 1,
          ),
          (starterCounts.get(starterKey) ?? 1) - 1)
        : undefined;
    return [
      {
...
function PlayoffWeekSchedule({
  ...
  const PlayoffMatchupCard = ({
    ...
    const venue =
      stadium ||
      (championshipNeutral ? "Neutral championship site" : home.stadium) ||
...
function ScheduleView({
  ...
  const scoreEntryDue =
    openScoreCount > 0 &&
    renderNow.getTime() >= new Date(nflWeekWindow.endsAt).getTime();
    ...
    <div className="workspace-stack">
      ...
      <div className="schedule-week-panel">
        ...
        >
          ...
          <div className="section-bar-actions">
            {!readOnlyHistory && week.weekNumber === liveWeekNumber && (
              <StakesButton
                schedule={schedule}
            ...
            {!readOnlyHistory ? (
              ...
              <span className="history-readonly-pill">
                <History />
                Provider history
              </span>
            )}
          ...
          {visibleGames.map((game) => {
            ...
            return (
              <MatchupCard
                ...
                teamHrefBase={teamHrefBase ?? `/season/${schedule.id}/team`}
            ...
            {visibleGames.map((game) => {
              ...
              return (
                ...
                >
                  <td>
                    <Link
                      href={`${leagueWeekHrefFor ? leagueWeekHrefFor(game.week) : `/season/${schedule.id}?week=${game.week}`}#${game.id}`}
                    >
                      W{game.week}
                    </Link>
                  </td>
                  ...
                  <td>
                    <TeamIdentityBlock
                      ...
                      href={
                        teamHrefFor
                          ? teamHrefFor(away.id)
                          : `/season/${schedule.id}/team/${away.id}`
                      }
                    />
                  </td>
                  <td>
                  ...
                  <td>
                    <TeamIdentityBlock
                      ...
                      href={
                        teamHrefFor
                          ? teamHrefFor(home.id)
                          : `/season/${schedule.id}/team/${home.id}`
                      }
                    />
                  </td>
                  <td>
...
function PlayoffsView({
        ...
        {team && item ? (
          <TeamIdentityBlock
            ...
            href={
              teamHrefFor
                ? teamHrefFor(team.id)
                : `/season/${schedule.id}/team/${team.id}`
            }
          />
        ) : (
          <>
  ...
  const lockField = () =>
    onUpdatePlayoffs(
      settings.fieldStatus === "locked"
        ...
        : {
            fieldStatus: "locked",
            lockedTeamIds: seeds.map((item) => item.teamId),
          },
    );
  const formatLabel = settings.bracketType
    .replaceAll("-", " ")
    .replace(/\b\w/g, (letter) => letter.toUpperCase());
      ...
      ? [
          {
            ...
            description:
              "Teams open inside their division, then cross for final places",
          },
        ]
      : []),
    {
  ...
  const MainPlayoffGame = ({
    ...
    const sideCopy =
      ...
      roundIndex < projectedMainRounds.length - 1 ? (
        <>
          {halfDivision && <DivisionMark division={halfDivision} size={13} />}
    ...
    return (
      <span
        ...
        style={
          {
            "--slot-c": slotColor,
            color: readableTextColor(slotColor),
          } as React.CSSProperties
        }
      >
        <b className="ppw-seed">{number}</b>
        {team?.logoUrl && (
          <img className="ppw-slogo" src={team.logoUrl} alt="" />
        )}
        <span className="ppw-name">
          {team ? teamDisplayName(team, showCity) : `Seed ${number}`}
        </span>
      </span>
    );
  };
  const wizardPreviewTitle =
    boardSection === "placement"
      ? "Where everyone finishes"
      : boardSection === "consolation"
        ? "Placement bracket"
        : "Road to the title";
  const wizardPreviewSub =
    boardSection === "placement"
  ...
  const wizardPreviewConnections: BracketConnection[] =
    boardSection === "championship"
      ? mainConnections
      : boardSection === "consolation"
        ? consolationInternalConnections
        : [];
  const wizardPreviewBracket =
    boardSection === "consolation" && consolationProjection
            ...
            entrants: [
              {
                ...
                label: "",
                color: seed(matchup.homeSeed)?.teamId
                  ? teamById.get(seed(matchup.homeSeed)!.teamId)?.color
                  : undefined,
              },
              {
                seed: matchup.awaySeed,
                label: "",
                color: seed(matchup.awaySeed)?.teamId
                  ? teamById.get(seed(matchup.awaySeed)!.teamId)?.color
                  : undefined,
              },
            ],
          })),
        ...
        <>
          <div className={`playoff-topline playoff-theme-${settings.theme}`}>
            <div>
              ...
              <span>
                ...
                <small>
                  {fieldSize} teams · {formatLabel} · Higher seed hosts before
                  title · {championshipVenueCopy}
                </small>
              </span>
            </div>
            <div className="playoff-field-actions">
              {simulationMode ? (
                ...
                <>
                  <span
                    className={`projected-pill ${settings.fieldStatus === "locked" ? "locked" : ""}`}
                  >
                    {settings.fieldStatus === "locked"
                      ? "FIELD LOCKED"
                      : "LIVE PROJECTION"}
                  </span>
                  <button type="button" onClick={lockField}>
                    <LockKeyhole />
                    {settings.fieldStatus === "locked"
                      ? "Unlock field"
                      : "Lock field"}
          ...
          <div className="playoff-policy">
            <span>
              ...
              <small>
                {placement === "division-halves"
                  ? `${sideName("A")} and ${sideName("B")} run separate tournaments; their champions meet in the final`
                  : placement === "division-leaders"
                    ? "Division winners protected at the top"
                    : "Top teams qualify regardless of division"}
              </small>
            </span>
            <span>
              <strong>
                {byeCount || "No"} {byeCount === 1 ? "bye" : "byes"}
              </strong>
              <small>
                {byeCount
                  ? "Awarded to the top seeds"
            ...
            <span>
              ...
              <small>
                {placement === "division-halves" &&
                settings.reseedMode !== "fixed"
                  ? "Reseeding stays inside each half until the final"
                  : settings.seedDisplayMode === "reranked"
                    ? "Showing bracket seeds"
                    : "Showing standings finish"}
              </small>
            </span>
          </div>
          {!simulationMode && (
            <>
              <div className="playoff-customization-bar">
                <span>
                  <strong>Postseason presentation</strong>
                  <small>
                    Choose placement format and personalize round names, game
                    names, and logos.
                  </small>
                </span>
                <CustomSelect
      ...
      <div className="platform-sync-card is-locked">
        <div>
          ...
          <span>
            ...
            <small>
              Auto-filling weekly scores from a public ESPN or Sleeper league is
              a Pro feature. Manual score entry is always available.
            </small>
          </span>
        </div>
      </div>
...
function HistoryMissingState({
  ...
  return (
    <section className="history-missing-state" role="status">
      <History />
      <span>
        <strong>
          {season
            ? `${season.season} schedule history is not saved yet.`
            : "No saved history is available yet."}
        </strong>
        <small>
          Previous years show only saved LeagueWeaver seasons or real
          ESPN/Sleeper provider history.
        </small>
      </span>
      {canSync && (
        <button type="button" onClick={onSync} disabled={syncing}>
          {syncing ? <LoaderCircle className="spin" /> : <RefreshCw />}Sync
          history
        </button>
      )}
    </section>
...
function SettingsView({
  ...
  const [tab, setTab] = useState<
    "overview" | "connections" | "rules" | "playoffs"
  >("overview");
  const seeding =
        ...
        <div className="settings-band">
          <div>
            ...
            <span>
              <strong>Standings tiebreakers</strong>
              <small>
                Tiebreaker settings are locked while this view is read-only.
              </small>
            </span>
          </div>
        </div>
        ...
        <section className="share-info-panel">
          ...
          <p className="share-card-summary">
            <span>
              <strong>{schedule.setup.name}</strong>
              <small>
                {schedule.setup.seasonYear} · {schedule.setup.teams.length}{" "}
                teams · {schedule.setup.weeks} weeks
              </small>
            </span>
          </p>
          <button
...
function ComingSoonWorkspace({
  ...
  return (
    <section
      className="workspace-coming-soon"
      aria-labelledby="workspace-coming-soon-title"
    >
      <span className="workspace-coming-soon-mark">
        <LockKeyhole />
      </span>
      <div>
      ...
      <span className="workspace-coming-soon-preview" aria-hidden="true">
        <Icon />
      </span>
    </section>
  );
...
function DraftRankingReminder({
  ...
  useEffect(() => {
    const update = () =>
      queueMicrotask(() => setBeforeWeekTwo(Date.now() < Date.parse(cutoff)));
    update();
...
export function SeasonWorkspace({
  ...
  const [gameDetailPlayerStats, setGameDetailPlayerStats] = useState<
    GameDetailPlayerStat[]
  >([]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const applyRows = (rows: GameDetailPlayerStat[]) => {
      if (!cancelled) setGameDetailPlayerStats(rows);
    };
    try {
      const cached = window.localStorage.getItem(
        `${GAME_DETAIL_CACHE_PREFIX}${schedule.id}`,
      );
      if (cached) {
        const parsed = JSON.parse(cached) as
          { rows?: GameDetailPlayerStat[] } | GameDetailPlayerStat[];
      ...
      fetch(`/api/seasons/${encodeURIComponent(schedule.id)}/player-stats`)
        ...
        .then((payload: { rows?: GameDetailPlayerStat[] } | null) => {
          if (!cancelled && payload?.rows)
            setGameDetailPlayerStats(payload.rows);
        })
        .catch(() => undefined);
    }
    return () => {
  ...
  const loadImportHistory = async () => {
    ...
    try {
      const response = await fetch(
        `/api/platform/history?scheduleId=${encodeURIComponent(schedule.id)}&include=browser`,
      );
      const payload = (await response.json().catch(() => ({}))) as {
        events?: ImportHistoryEvent[];
        champions?: PastChampion[];
        history?: HistoryBrowserSeason[];
        error?: string;
      };
      if (!response.ok)
        throw new Error(
  ...
  useEffect(() => {
    ...
    if (Number.isInteger(week) && week >= 1 && week <= 17)
      queueMicrotask(() => setSelectedWeek(week));
    ...
    if (requestedViewValue === "fairness")
      queueMicrotask(() => setView("league-schedule"));
    const requestedView = (
    ...
    if (HISTORY_COMPATIBLE_VIEWS.has(historyTargetView))
      queueMicrotask(() =>
    ...
    if (
      ...
      (requestedView !== "results" ||
        (schedule && getLatestFinalWeek(schedule)))
    )
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      ...
      try {
        ...
        const parsed = stored
          ? (JSON.parse(stored) as SimulationSandbox)
          : null;
        setSavedSimulation(
  ...
  const historyOptions = useMemo(
    () => [
      {
        value: "current",
        label: `${schedule?.setup.seasonYear ?? "Current"}`,
        description: "Current LeagueWeaver season",
      },
      ...historySeasons
        ...
        .map((season) => ({
          ...
          description: `${season.provider.toUpperCase()} saved history`,
        })),
    ],
    [historySeasons, schedule?.setup.seasonYear],
  );
  const effectiveHistorySeasonKey =
    historySeasonKey === "current" ? "current" : historySeasonKey;
  const selectedHistorySeason = useMemo(
    () =>
      effectiveHistorySeasonKey === "current"
        ? undefined
        : historySeasons.find(
            (season) => String(season.season) === effectiveHistorySeasonKey,
          ),
    [effectiveHistorySeasonKey, historySeasons],
  );
  const historySchedule = useMemo(
    () =>
      schedule && selectedHistorySeason
        ? buildHistoricalSchedule(schedule, selectedHistorySeason)
        : null,
    [schedule, selectedHistorySeason],
  );
  const historyPlayerStats = useMemo(
    () =>
      schedule && selectedHistorySeason
        ? buildHistoricalPlayerRows(schedule, selectedHistorySeason)
        : [],
    [schedule, selectedHistorySeason],
  );
  ...
  useEffect(() => {
    if (
      !workspaceSchedule ||
      !selectedTeamId ||
      workspaceSchedule.setup.teams.some((team) => team.id === selectedTeamId)
    )
      return;
    queueMicrotask(() =>
      setSelectedTeamId(workspaceSchedule.setup.teams[0]?.id ?? ""),
    );
  }, [workspaceSchedule, selectedTeamId]);
  const latestRecapWeek = activeSchedule
    ? getLatestFinalWeek(activeSchedule)
    : null;
  const visibleViewItems = VIEW_ITEMS.filter(
    (item) => item.key !== "results" || latestRecapWeek,
  );
  const openGameDetail = (gameId: string) => {
    if (!workspaceSchedule) return;
    const gameWeek = workspaceSchedule.weeks.find((item) =>
    ...
    try {
      const response = await fetch("/api/platform/history", {
        ...
        body: JSON.stringify({
          ...
          identifier: connection.providerLeagueId,
  ...
  const savePlatformConnection = async (
    ...
    try {
      ...
      const result = (await response.json().catch(() => ({}))) as {
        ...
        historySync?: HistorySyncResponse;
        error?: string;
      };
      if (!response.ok)
        throw new Error(
      ...
      await loadImportHistory();
      setNotice(
        `Platform connection saved.${historySyncNotice(result.historySync)}`,
      );
    } catch (caught) {
      setNotice(
        caught instanceof Error
    ...
    if (entitlements.signedIn && CLOUD_SCHEDULE_ID.test(schedule.id)) {
      void fetch("/api/platform/connections", {
        ...
        .then(async (response) => {
          ...
          if (historyNotice)
            setNotice(
              `Connected to ${connection.provider === "espn" ? "ESPN" : "Sleeper"}.${historyNotice} Pulling scores…`,
            );
        })
        .catch(() => undefined);
    }
  ...
  const onUpdatePublicDisplay = (display: PublicDisplaySettings) => {
    ...
    setPublishStatus((current) =>
      current ? { ...current, publicDisplay: display } : current,
    );
  };
  ...
  const historyViewHref = (key: ViewKey) => {
    ...
    if (
      HISTORY_COMPATIBLE_VIEWS.has(key) &&
      effectiveHistorySeasonKey !== "current"
    )
      params.set("season", effectiveHistorySeasonKey);
  ...
  const historySeasonParam =
    historyViewActive && effectiveHistorySeasonKey !== "current"
      ? `season=${encodeURIComponent(effectiveHistorySeasonKey)}`
      : "";
  const hrefWithHistorySeason = (
    path: string,
  ...
  const selectView = (item: (typeof VIEW_ITEMS)[number]) => {
    ...
    if (item.key === "league-schedule" && selectedWeek > schedule.weeks.length)
      setSelectedWeek(1);
    setView(item.key);
    if (HISTORY_COMPATIBLE_VIEWS.has(item.key))
      router.push(historyViewHref(item.key));
  ...
  const selectTeamSchedule = (teamId: string) => {
    ...
    router.push(
      teamId
        ...
        : hrefWithHistorySeason(`/season/${schedule.id}`, {
            view: "team-schedule",
          }),
    );
  };
  ...
  const scoreBarTeamById = new Map(
    scoreBarSchedule.setup.teams.map((team) => [team.id, team]),
  );
  const scoreBarRankByTeam = new Map(
    (
      getEnteringWeekRankSnapshot(scoreBarSchedule, selectedWeek)?.rows ?? []
    ).map((row) => [row.teamId, row.rank]),
  );
  const scoreBarDivisionById = new Map(
    scoreBarSchedule.setup.divisions.map((division) => [division.id, division]),
  );
  const scoreBarWeek =
    scoreBarSchedule.weeks.find((item) => item.weekNumber === selectedWeek) ??
    scoreBarSchedule.weeks[0];
  const modalWeek =
    gameDetailId && workspaceSchedule
      ? workspaceSchedule.weeks.find((week) =>
          week.games.some((game) => game.id === gameDetailId),
        )
      : undefined;
  const modalPlayoffGame =
    gameDetailId && workspaceSchedule
      ? (workspaceSchedule.playoffGames ?? []).find(
          (game) => game.id === gameDetailId,
        )
      : undefined;
  const modalGames =
    modalWeek?.games ??
    (modalPlayoffGame && workspaceSchedule
      ? (workspaceSchedule.playoffGames ?? [])
          .filter(
            (game) =>
              game.week === modalPlayoffGame.week &&
              game.bracket === modalPlayoffGame.bracket,
          )
          .sort(
            (left, right) =>
              (left.gameNumber ?? 0) - (right.gameNumber ?? 0) ||
      ...
      : undefined);
  const modalGameIndex =
    modalGames?.findIndex((game) => game.id === gameDetailId) ?? -1;
  const modalGameLabel = (game?: ScheduledGame) => {
    if (!game || !workspaceSchedule) return "";
    const away = workspaceSchedule.setup.teams.find(
      (team) => team.id === game.awayTeamId,
    );
    const home = workspaceSchedule.setup.teams.find(
      (team) => team.id === game.homeTeamId,
    );
    return away && home
      ? `${teamDisplayName(away, workspaceSchedule.setup.display?.cityNames !== false)} at ${teamDisplayName(home, workspaceSchedule.setup.display?.cityNames !== false)}`
      : "game";
  };
  const modalPreviousGame =
    modalGames && modalGameIndex > 0
      ? modalGames[modalGameIndex - 1]
      : undefined;
  const modalNextGame =
    modalGames && modalGameIndex >= 0 && modalGameIndex < modalGames.length - 1
      ? modalGames[modalGameIndex + 1]
      : undefined;
  const canSyncHistory = Boolean(
    schedule.setup.platformConnection &&
    CLOUD_SCHEDULE_ID.test(schedule.id) &&
    !simulation,
  );
    ...
    >
      ...
      {gameDetailId && (
        <GameDetailSheet
          ...
          navigation={
            modalGames
              ? {
                  previous: modalPreviousGame
                    ? {
                        id: modalPreviousGame.id,
                        label: modalGameLabel(modalPreviousGame),
                      }
                    : undefined,
                  next: modalNextGame
                    ? {
      ...
      <header className="workspace-topbar">
        ...
        <div className="workspace-top-actions">
          <AccountIdentity identity={entitlements} plan={entitlements.plan} />
        </div>
      </header>
      {scoreBarWeek && (
        <WeekScoreBar
      ...
      <div className="workspace-shell">
        ...
        >
          ...
          <div className="workspace-content">
                ...
                <ThisWeekWorkspace
                  ...
                  onOpenRecap={
                    latestRecapWeek
                      ? () => {
                          setView("results");
                          router.push(`/season/${schedule.id}?view=results`);
                        }
                      : undefined
                  }
                />
            ...
            !historySchedule ? (
              ...
              view === "league-schedule" && (
                <ScheduleView
                  ...
                  highlightedGame={highlightedGame}
                  simulationResults={
                    historyViewActive ? {} : simulationResultByGame
                  }
                  simulationProbabilities={
                    historyViewActive ? {} : simulationProbabilityByGame
                  }
            ...
            {view === "mvt" && (
              <ComingSoonWorkspace
                title="MVT is coming soon"
                description="This award workspace is locked while we finish the next version."
                icon={Medal}
              />
            )}
            {view === "all-stars" && (
              <ComingSoonWorkspace
                title="All-Stars is coming soon"
                description="This season honors page is locked while we finish the next version."
                icon={Sparkles}
              />
            )}
            {view === "playoffs" && historyViewActive && !historySchedule ? (
              ...
              view === "playoffs" && (
                <PlayoffsView
                  ...
                  highlightedGame={highlightedGame}
                  simulationMode={Boolean(simulation || historyViewActive)}
