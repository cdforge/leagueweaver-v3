...
import { conferenceDivisionAcronym, divisionAcronym, leagueAcronym, resolveInitials } from "@/lib/monograms";
...
export function StatsWorkspace({ schedule, onUpdateTiebreakers, readOnly = false }: { schedule: GeneratedSchedule; onUpdateTiebreakers?: (settings: TiebreakerSettings) => void; readOnly?: boolean }) {
  ...
  const filterOptions = [{ value: "all", label: "League standings", description: `${schedule.setup.teams.length} teams`, swatch: schedule.setup.color, logoUrl: schedule.setup.logoUrl, monogram: resolveInitials(schedule.setup.initials, leagueAcronym(schedule.setup.name)), entityType: "league" as const }, ...schedule.setup.divisions.map((division) => { const conference = conferenceOfDivision(schedule.setup, division.id); return { value: division.id, label: conference ? `${conference.name} · ${division.name}` : division.name, description: `${schedule.setup.teams.filter((team) => team.divisionId === division.id).length} teams`, swatch: division.color, logoUrl: division.logoUrl, monogram: conference ? conferenceDivisionAcronym(division.name, division.initials, conference.name, conference.initials) : resolveInitials(division.initials, divisionAcronym(division.name)), entityType: "division" as const }; })];
