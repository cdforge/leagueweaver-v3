...
export function StatsWorkspace({ schedule, onUpdateTiebreakers, readOnly = false }: { schedule: GeneratedSchedule; onUpdateTiebreakers?: (settings: TiebreakerSettings) => void; readOnly?: boolean }) {
  ...
  const divisionConference = (division: Division) => leagueHasConferences ? conferenceOfDivision(schedule.setup, division.id) : undefined;
  const divisionOptionMonogram = (division: Division) => {
    const conference = divisionConference(division);
    return conference
