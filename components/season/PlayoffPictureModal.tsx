...
import type { Conference, Division, GeneratedSchedule } from "@/lib/types";
...
export function PlayoffPicturePanel({ schedule, onClose }: { schedule: GeneratedSchedule; onClose?: () => void }) {
  ...
  const conferenceForDivision = (division?: Division): Conference | undefined => division ? conferenceOfDivision(schedule.setup, division.id) : undefined;

  const StatusTag = ({ entry, division }: { entry: PlayoffPictureEntry; division?: Division }) => {
    const conference = conferenceForDivision(division);
    if (entry.status === "top-seed") return <span className="pp-tag is-top"><Medal aria-hidden="true" />#1 Seed</span>;
    if (entry.status === "division") return <span className="pp-tag">{division && <DivisionMark division={division} conference={conference} size={13} className="pp-dmark" />}Div Champ</span>;
        ...
        <div className={`pp-srow${entry.isTopSeed ? " is-top" : ""}`} style={{ "--tc": team.color, "--bar": accessibleTeamColor(team.color), "--dc": division?.color ?? "#5b6b64" } as CSSProperties}>
          ...
          <div className="pp-bar">
            <div className="pp-tm">
              ...
              {division && <span className="pp-div"><DivisionMark division={division} conference={conferenceForDivision(division)} size={13} className="pp-dmark" />{division.name}</span>}
            </div>
    ...
    return (
      <div className={`pp-mrow${out ? " is-out" : ""}`} style={{ "--tc": team.color, "--dc": division?.color ?? "#5b6b64" } as CSSProperties}>
        ...
        <div className="pp-tm2">
          ...
          {division && <span className="pp-mdv"><DivisionMark division={division} conference={conferenceForDivision(division)} size={12} className="pp-dmark" />{division.name}</span>}
        </div>
        <span className="pp-sp" />
