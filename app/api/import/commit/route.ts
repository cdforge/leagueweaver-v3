...
const commitSchema = z.object({
  ...
  teams: z.array(z.object({ name: z.string().trim().min(1) })).min(8).max(32),
  requiresConfirmation: z.literal(true),
});

export async function POST(request: Request) {
  const parsed = commitSchema.safeParse(await request.json());
  if (!parsed.success || parsed.data.teams.length % 2 !== 0) return NextResponse.json({ error: "Confirm an even roster of 8–32 named teams before importing." }, { status: 400 });
