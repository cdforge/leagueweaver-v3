...
export type SchedulePdfProvider = "espn" | "sleeper";
export type SchedulePdfMode = "schedule" | "edit";

export type SchedulePdfOptions = {
  provider?: SchedulePdfProvider;
  mode?: SchedulePdfMode;
};
...
export async function downloadSchedulePdf(schedule: GeneratedSchedule, options: SchedulePdfOptions = {}) {
  const provider = options.provider ?? "espn";
  const mode = options.mode ?? "schedule";
  const providerLabel = provider === "espn" ? "ESPN" : "Sleeper";
  const modeLabel = provider === "espn" && mode === "edit" ? "edit matchup" : "schedule";
  const title = `${providerLabel} ${modeLabel} handoff sheet`;
  const instruction = provider === "espn"
    ? "Use with ESPN LM Tools > Edit Head-to-Head Schedule"
    : "Use with Sleeper Settings > Commissioner Control > Edit Schedule Matchups";
  ...
  drawBrandHeader();
  y = pageHeight - 70;
  page.drawText(title, { x: margin, y, font: displayFont, size: 28, color: ink });
  y -= 17;
  page.drawText(`${schedule.setup.name} · ${schedule.setup.teams.length} teams · ${schedule.setup.weeks} weeks · ${instruction}`, { x: margin, y, font: bodyFont, size: 9, color: gray });
  y -= 13;
  ...
  pages.forEach((pdfPage, index) => {
    pdfPage.drawText(`Generated by League Weaver · ${providerLabel} handoff sheet · Page ${index + 1} of ${pages.length}`, { x: margin, y: 20, font: bodyFont, size: 7, color: gray });
  });
  ...
  anchor.href = url;
  anchor.download = `${schedule.setup.abbreviation.toLowerCase()}-${schedule.setup.seasonYear}-${provider}-${modeLabel.replaceAll(" ", "-")}-handoff.pdf`;
  anchor.click();
